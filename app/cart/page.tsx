    ...
    <main className="mx-auto w-full max-w-6xl px-4 pb-16 pt-10 sm:px-6 md:px-8 lg:px-12">
      ...
      {cartItems.length === 0 ? (
        <section className="elevated-card mt-8 rounded-lg p-8">
          ...
          <Link
            href="/products"
            className="mt-4 inline-flex rounded-md bg-foreground px-4 py-2 text-sm text-white"
          >
            Browse Products
          </Link>
        </section>
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[1fr_320px]">
          ...
          <aside className="elevated-card h-fit rounded-lg p-6">
            ...
            <Link
              href={cartItems.length > 0 ? `/checkout/${cartItems[0].template.id}` : "/products"}
            ...
            <p className="mt-2 text-xs text-muted">Bundle checkout starts with the first cart item in this demo.</p>
          </aside>
        </section>
      )}
